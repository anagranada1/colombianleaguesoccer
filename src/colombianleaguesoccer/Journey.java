...
public class League {
    ...
    private List<Journey> createEmptyJourneys() {
    ...
    private void fillJourney(Journey journey, List<Team> pool) {
        for (Team copyTeam : pool) {
            ...
            scheduleMatchesFor(journey, copyTeam, pool);
        }
    }

    private void scheduleMatchesFor(Journey journey, Team copyTeam, List<Team> pool) {
        for (Team rival : pool) {
    ...
    private void rotateLeft(List<Team> list) {
        Team first = list.remove(0);
        list.add(first);
    }
